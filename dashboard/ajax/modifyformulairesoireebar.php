...
require_once('../includes/connectBDD.php');
        ...
        if(!empty($user_id)&&!empty($adresse)&&!empty($codepostal)&&!empty($ville)&&!empty($tel)&&!empty($telurgence)){
                $update = $db->prepare("UPDATE formulairesoireebar SET adresse='$adresse', codepostal='$codepostal', ville='$ville', tel='$tel', telurgence='$telurgence' WHERE user_id='$user_id'");
                $update->execute();
                ?>
            <script>
